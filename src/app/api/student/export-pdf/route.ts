...
export async function POST() {
  try {
    ...
    // Page 3 (landscape school details)
    ...
    for (const cls of ['VI', 'VII', 'VIII', 'IX', 'X', 'XI', 'XII']) {
      ...
      for (let i = 0; i < values.length; i++) {
        const w = colX[i + 1] - colX[i];
        p3.drawRectangle({ x: colX[i], y, width: w, height: rowH3, borderWidth: 1, borderColor: rgb(0, 0, 0) });
        drawCellText(p3, font, values[i], colX[i], y, w, rowH3, 8.5);
      }
      y -= rowH3;
    }
 
    // Page 4 - Marks + relative details + signatures
    ...
    const fileName = `${sanitizeText(student.full_name || 'student').replace(/\s+/g, '_')}_${new Date()
      .toISOString()
      .split('T')[0]}.pdf`;
    ...
    console.error('Student PDF generation error:', e);
